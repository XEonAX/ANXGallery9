...
public class PreferenceCleaner {
    ...
    private static void transferOldPreference() {
        if (!PreferenceHelper.getBoolean(PrefKeys.UPGRADE_OLD_PREFERENCES_TRANSFERED, false)) {
            ...
            boolean sGetImpunityDeclarationEveryTime = true ^ Preference.sGetImpunityDeclarationEveryTime();
            if (Preference.sCanConnectNetworkByImpunity() && sGetImpunityDeclarationEveryTime) {
                CTA.setCanConnectNetwork(sGetImpunityDeclarationEveryTime);
            }
        }
    }
}
