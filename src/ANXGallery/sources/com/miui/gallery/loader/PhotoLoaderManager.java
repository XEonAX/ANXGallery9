...
public class PhotoLoaderManager {
    ...
    public BaseLoader getPhotoDataSet(Context context, Uri uri, Bundle bundle) {
        if (uri != null) {
            ...
            while (it.hasNext()) {
                PhotoLoaderSource photoLoaderSource = (PhotoLoaderSource) it.next();
                if (photoLoaderSource.match(uri, bundle)) {
                    return photoLoaderSource.createDataLoader(context, uri, bundle);
                }
            }
        }
        return null;
