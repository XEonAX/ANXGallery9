...
public class BackDownloadJob extends AbsSyncJob<Void> {
    ...
    public JobInfo getJobInfo(Context context, ComponentName componentName) {
        return new Builder(getJobId(), componentName).setRequiredNetworkType(2).setRequiresDeviceIdle(CloudControlStrategyHelper.getSyncStrategy().isAutoDownloadRequireDeviceIdle()).setRequiresCharging(true).build();
    }
}
