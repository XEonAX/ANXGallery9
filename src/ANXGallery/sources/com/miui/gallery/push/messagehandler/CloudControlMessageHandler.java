...
public class CloudControlMessageHandler extends MessageHandler {
    ...
    public void handleDirect(Context context, GalleryPushMessage galleryPushMessage) {
        ...
        long pushTag = CloudControl.getPushTag();
        boolean z = false;
        if (tag <= pushTag) {
        ...
        CloudControl.setPushTag(tag);
        String data = galleryPushMessage.getData();
        try {
            ArrayList arrayList = (ArrayList) new GsonBuilder().registerTypeAdapter(FeatureProfile.class, new Deserializer()).create().fromJson(data, new TypeToken<ArrayList<FeatureProfile>>() {
            }.getType());
