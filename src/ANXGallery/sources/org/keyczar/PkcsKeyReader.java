...
public class PkcsKeyReader implements KeyczarReader {
    ...
    public PkcsKeyReader(KeyPurpose keyPurpose, InputStream inputStream, RsaPadding rsaPadding2, String str) throws KeyczarException {
        if (keyPurpose == null) {
            throw new KeyczarException("Key purpose must not be null");
        } else if (inputStream != null) {
            this.purpose = keyPurpose;
            this.pkcs8Stream = inputStream;
            this.rsaPadding = rsaPadding2;
            this.passphrase = str;
        } else {
    ...
    private static KeyczarKey parseKeyStream(InputStream inputStream, String str, RsaPadding rsaPadding2) throws IOException, KeyczarException {
        ...
        try {
            return new RsaPrivateKey((RSAPrivateCrtKey) extractPrivateKey(pKCS8EncodedKeySpec, "RSA"), rsaPadding2);
        } catch (InvalidKeySpecException unused) {
            DsaPrivateKey dsaPrivateKey = new DsaPrivateKey((DSAPrivateKey) extractPrivateKey(pKCS8EncodedKeySpec, "DSA"));
            if (rsaPadding2 == null) {
                return dsaPrivateKey;
            }
            throw new KeyczarException(Messages.getString("InvalidPadding", rsaPadding2.name()));
